...
RRRG2OWrapper::Result RRRG2OWrapper /*<VertexType,EdgeType>*/ ::optimize(
    const EdgeIdSet &active_loops, const int n_iterations)
{
  parseGraph();
  ...
  for (auto &&loop_id : active_loops) {
    ...
    res.loop_errors_.emplace(
        std::make_pair(loop_id, loop_edges_[loop_id]->chi2()));
  }
...
{
  g2o::OptimizableGraph::EdgeSet::iterator it = optimizer_->edges().begin(),
