...
namespace internal
{
    struct EdgeCov
    {
      ...
      size_t hop_;
      EdgeCov() : node_id_(0), cov_(Covar::Zero()),t_(Transform::Identity()), hop_(0),det_cov_(0)
      {
      }
      explicit EdgeCov(size_t start_id)
        : start_id_(start_id), cov_(), t_(Transform::Identity()),hop_(0), det_cov_(0)
      {
      }
      ...
      EdgeCov operator+(const EdgeCov &new_e) const{
        ...
        e.cov_ = computeJacc1(co, si, new_e.t_.translation()) *
                     this->cov_ * computeJacc1(co, si, new_e.t_.translation())
                         .transpose() + computeJacc2(co,si) * new_e.cov_ * computeJacc2(co,si).transpose();
      ...
      private:
       ...
       Eigen::Matrix3d computeJacc1(double co, double si ,const Eigen::Vector2d & t_j) const{
        Eigen::Matrix3d res;
        res<< 1, 0 , -si*t_j(0)-co*t_j(1), 0, 1 , co*t_j(0) - si*t_j(1), 0,0,1;
        return res;
       }
...
struct LoopClosure
{
  ...
  LoopClosure(size_t node_from, size_t node_to,
    ...
    : vertices_(std::make_pair(node_from,node_to)), information_(inf), t_(trans)
  {
  }
};
