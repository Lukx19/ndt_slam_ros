...
protected:
  ...
  virtual bool computeSingleGrid(PclSource &output, TargetGrid &target_grid,
                                 const Eigen::Matrix4f &guess);
  ...
  calcScore(TargetGrid &ndt_grid, const PclSource &source,
...
template <typename PointSource, typename PointTarget>
NormalDistributionsTransform2DEx<
    PointSource, PointTarget>::NormalDistributionsTransform2DEx()
  : target_cells_()
  , resolution_(1.0f)
  , step_size_(0.1)
  , outlier_ratio_(0.55)
...
template <typename PointSource, typename PointTarget>
bool
NormalDistributionsTransform2DEx<PointSource, PointTarget>::computeSingleGrid(
    PclSource &output, TargetGrid &target_grid, const Eigen::Matrix4f &guess)
{
  nr_iterations_ = 0;
  converged_ = false;
  ...
  while (!converged_) {
    ...
    Eigen::JacobiSVD<Eigen::Matrix3d> sv(
        score.hessian, Eigen::ComputeFullU | Eigen::ComputeFullV);
    ...
    if (delta_p_norm == 0 || delta_p_norm != delta_p_norm) {
      trans_probability_ =
          score.value / static_cast<double>(input_->points.size());
      converged_ = delta_p_norm == delta_p_norm;
      covariance_.setIdentity();
      inform_matrix_.setIdentity();
      ROS_ERROR_STREAM(
          "Not enough overlap. Probability: " << trans_probability_);
      return false;
    }
    ...
    ROS_DEBUG_STREAM("[NDT2D]: Step: "
                     << delta_p_norm << " Delta: "
                     << delta_xytheta_p.transpose() << " Score: " << score.value
...
NormalDistributionsTransform2DEx<PointSource, PointTarget>::calcScore(
    TargetGrid &ndt_grid, const PclSource &source, const Eigen::Vector3d &trans)
{
  ...
  for (size_t i = 0; i < trans_source.size(); ++i) {
    ...
    ndt_grid.radiusSearch(trans_source[i], resolution_, neighborhood,
                          distances);
    size_t count = 0;
    for (typename NeighborsVector::iterator neighborhood_it =
             neighborhood.begin();
         neighborhood_it != neighborhood.end(); neighborhood_it++) {
      count++;
      res += calcPointScore(source[i], *neighborhood_it, trans_source[i],
                            cos_theta, sin_theta);
    }
...
NormalDistributionsTransform2DEx<PointSource, PointTarget>::computeStepLengthMT(
    ...
    double step_init, double step_max, double step_min, TargetGrid &ndt_grid,
    const ndt2d::ScoreAndDerivatives<3, double> &score, const PclSource &input)
{
